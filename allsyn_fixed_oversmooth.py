...
def multiprocess_TAKDE(frame=100,window = 4, mc = 30, rule = "normal", weighting = "oversmooth",fixedwindow=True):
    import numpy as np
    log = np.empty(mc)
    n=500
    ...
    for i in range(mc):
        ...
        ests = estimator.Streaming_Estimation(df_1_test,df_1t,width_selector = rule, weighting = weighting,
                                              t=window,fixedwindow=fixedwindow)
        testlog = 0
...
def main():
    ...
    if rank ==0:
        ...
        if size > len(iter_list):
            ...
            savetxt('/scratch/wang.yinso/cps2021/simulation/synthetic/synthetic_fixed_oversmooth.csv',cv_result,delimiter=',')
        else:
        ...
        for i in range(len(locallist)):
            ...
            frame = locallist[i][1]
            log,ste = multiprocess_TAKDE(frame=frame,window=cutoff)
            result[0] = cutoff
            result[1] = frame
