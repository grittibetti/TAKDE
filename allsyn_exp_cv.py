...
def multiprocess_TAKDE(frame=100,cutoff = 4, mc = 100, rule = "cv", weighting = "exponential",cv=0.1):
    import numpy as np
    log = np.empty(mc)
    n=500
...
def main():
    ...
    if rank ==0:
        ...
        if size > len(iter_list):
            ...
            savetxt('/scratch/wang.yinso/cps2021/simulation/synthetic/synthetic_exponential_cv.csv',cv_result,delimiter=',')
        else:
        ...
        for i in range(len(locallist)):
            ...
            cv = locallist[i][1]
            log,ste = multiprocess_TAKDE(cv=cv,cutoff=cutoff)
            result[0] = cutoff
            result[1] = cv
