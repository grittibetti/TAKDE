...
def multiprocess_TAKDE(frame=100,cutoff = 4, mc = 100, rule = "cv", weighting = "average",cv=0.1):
    import numpy as np
    log = np.empty(mc)
    n=500
...
def main():
    ...
    if rank ==0:
        ...
        if size > len(iter_list):
            ...
            savetxt('/scratch/wang.yinso/cps2021/simulation/synthetic/synthetic_average_cv.csv',cv_result,delimiter=',')
        else:
