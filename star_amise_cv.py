...
def multiprocess_TAKDE(data,testlist,cutoff = 4, mc = 50, rule = "cv", weighting = "amise",cv = 0.2):
    import numpy as np
    log = np.empty(mc)
    for i in range(mc):
...
def main():
    ...
    size = comm.Get_size()
    if rank ==0:
        tsv_data = pd.read_csv("/scratch/wang.yinso/cps2021/StarLightCurves_TRAIN.tsv",sep='\t')
        ...
        cut_list = np.arange(0.2,4,0.2)
        cv_list = np.arange(0.05,1.1,0.1)
        cutoff_list = list(product(cut_list,cv_list))
        ...
        if size > len(cutoff_list):
            ...
            savetxt('/scratch/wang.yinso/cps2021/simulation/star/cvresult_amise_cv.csv',cv_result,delimiter=',')
        else:
        ...
        for i in range(len(locallist)):
            ...
            cv = locallist[i][1]
            log,ste = multiprocess_TAKDE(localdata,testlist,cutoff=cutoff,cv=cv)
            result[0] = cutoff
            result[1] = cv
            result[2] = log
            result[3] = ste
