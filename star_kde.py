...
def main():
    ...
    if rank ==0:
        ...
        if size > len(cutoff_list):
            ...
            savetxt('/scratch/wang.yinso/cps2021/simulation/star/cvresult_kde.csv',cv_result,delimiter=',')
        else:
        ...
        for i in range(len(locallist)):
            ...
            cv = locallist[i][1]
            log,ste = multiprocess_TAKDE(localdata,testlist,w=cutoff,cv=cv)
            result[0] = cutoff
            result[1] = cv
            result[2] = log
            result[3] = ste
