...
def multiprocess_TAKDE(data,testlist,cutoff = 4, mc = 100, rule = "cv", weighting = "amise",cv = 0.2):
    import numpy as np
    log = np.empty(mc)
    for i in range(mc):
...
def main():
    ...
    if rank ==0:
        ...
        if size > len(cutoff_list):
            ...
            savetxt('/scratch/wang.yinso/cps2021/simulation/wafer/cvresult_amise_cv.csv',cv_result,delimiter=',')
        else:
